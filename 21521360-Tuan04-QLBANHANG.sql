...
SELECT K.MAKH, HOTEN FROM KHACHHANG K , HOADON H WHERE K.MAKH = H.MAKH AND NGHD = '1/1/2007';


--9.	In ra số hóa đơn, trị giá các hóa đơn do nhân viên có tên “Nguyen Van B” lập trong ngày 28/10/2006.
SELECT SOHD, TRIGIA FROM HOADON H INNER JOIN NHANVIEN N ON H.MANV = N.MANV WHERE NGHD = '10/28/2006' AND HOTEN = 'NGUYEN VAN B'
SELECT SOHD,TRIGIA FROM HOADON H,NHANVIEN N WHERE H.MANV=N.MANV AND NGHD = '10/28/2006' AND HOTEN = 'NGUYEN VAN B'
...
SELECT DISTINCT S.MASP,TENSP FROM (SANPHAM S INNER JOIN CTHD C 
										INNER JOIN HOADON D
											INNER JOIN KHACHHANG H
													ON D.MAKH=H.MAKH
												ON C.SOHD=D.SOHD
											ON S.MASP=C.MASP)
									WHERE HOTEN='Nguyen Van A' AND MONTH(NGHD)=10 AND YEAR(NGHD)=2006

SELECT S.MASP,TENSP FROM SANPHAM S,CTHD C,HOADON D,KHACHHANG H WHERE S.MASP=C.MASP AND C.SOHD=D.SOHD AND D.MAKH=H.MAKH AND HOTEN='Nguyen Van A' 
...
SELECT DISTINCT S.MASP,TENSP FROM SANPHAM S,CTHD C,HOADON H WHERE S.MASP=C.MASP AND C.SOHD=H.SOHD AND (NUOCSX='TRUNG QUOC' OR NGHD='1/1/2007')
...
SELECT MASP,TENSP FROM SANPHAM EXCEPT (SELECT DISTINCT C.MASP,S.TENSP FROM CTHD C,SANPHAM S WHERE C.MASP=S.MASP) 
...
SELECT COUNT(*) 'So hoa don'
FROM HoaDon
WHERE MaKH IS NULL

--21.	Có bao nhiêu sản phẩm khác nhau được bán ra trong năm 2006.
SELECT  COUNT(DISTINCT C.MASP) FROM CTHD C,HOADON D WHERE  C.SOHD=D.SOHD AND YEAR(NGHD)=2006
...
SELECT AVG(TRIGIA)'Trung Binh' FROM HOADON WHERE YEAR(NGHD)=2006

--24.	Tính doanh thu bán hàng trong năm 2006.
SELECT SUM(TRIGIA)'Doanh Thu' FROM HOADON WHERE YEAR(NGHD)=2006

--25.	Tìm số hóa đơn có trị giá cao nhất trong năm 2006.
SELECT SOHD FROM HOADON WHERE TRIGIA=(SELECT MAX(TRIGIA) FROM HOADON WHERE YEAR(NGHD)=2006)

--26.	Tìm họ tên khách hàng đã mua hóa đơn có trị giá cao nhất trong năm 2006.
SELECT DISTINCT HOTEN FROM KHACHHANG H,HOADON D WHERE H.MAKH=D.MAKH AND TRIGIA=(SELECT MAX(TRIGIA) FROM HOADON WHERE YEAR(NGHD)=2006)
...
SELECT NUOCSX,COUNT(MASP) 'So San Pham' FROM SANPHAM 
GROUP BY NUOCSX

--34.	Với từng nước sản xuất, tìm giá bán cao nhất, thấp nhất, trung bình của các sản phẩm.
SELECT NUOCSX ,MAX(GIA) 'Cao nhat', MIN(GIA) 'Thap nhat',AVG(GIA) 'Trung Binh' FROM SANPHAM 
GROUP BY NUOCSX
...
SELECT MASP,SUM(SL) 'So luong ban' FROM HOADON D,CTHD C WHERE D.SOHD=C.SOHD AND MONTH(NGHD) = 10 AND YEAR(NGHD)=2006
GROUP BY MASP
...
SELECT SOHD 'sohoadon',COUNT(DISTINCT MASP)'Số lượng sản phẩm' FROM CTHD
GROUP BY SOHD HAVING COUNT(DISTINCT MASP) >= 4

--39.	Tìm hóa đơn có mua 3 sản phẩm do “Viet Nam” sản xuất (3 sản phẩm khác nhau).
SELECT C.SOHD 'Số hóa đơn',COUNT(DISTINCT C.MASP)'Số lượng sản phẩm' FROM CTHD C,SANPHAM S WHERE C.MASP=S.MASP AND NUOCSX='Viet Nam'
GROUP BY SOHD HAVING COUNT(DISTINCT C.MASP) >= 3


--40.	Tìm khách hàng (MAKH, HOTEN) có số lần mua hàng nhiều nhất. 
SELECT D.MAKH,COUNT(SOHD) 'Số lần mua hàng' FROM KHACHHANG H, HOADON D WHERE H.MAKH=D.MAKH
GROUP BY D.MAKH HAVING COUNT(*) >= ALL(SELECT COUNT(*) FROM HOADON GROUP BY MaKH)


--41.	Tháng mấy trong năm 2006, doanh số bán hàng cao nhất ?
SELECT  MONTH(NGHD)'Thang' ,SUM(TRIGIA) DoanhThu FROM HOADON WHERE YEAR(NGHD)=2006
GROUP BY MONTH(NGHD) HAVING SUM(TRIGIA) >=ALL(SELECT SUM(TriGia) FROM HoaDON WHERE YEAR(NgHD) = 2006 GROUP BY MONTH(NgHD))
